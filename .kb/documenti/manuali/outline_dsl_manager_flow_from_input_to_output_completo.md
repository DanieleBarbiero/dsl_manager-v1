# DSL Manager: dal dato grezzo all'output — outline classico
...
Il primo comando crea la struttura; il secondo crea e migra `workspace.sqlite`. Nella release 1.1, `db init` applica le migrazioni fino alla v10. Il registry SQLite è il vero centro persistente dell’applicazione: il DSL finale sarà soltanto una sua vista derivata e riproducibile.
...
(nota storica: la conversazione originale anticipava l'aggiornamento v02. In questa edizione per la release 1.1, i passaggi descritti sono stati riallineati allo stato implementato e alla documentazione canonica.)
...
Nella release 1.1, `.xlsx` e `.xlsm` vengono letti direttamente, senza
conversione. Macro e collegamenti esterni non vengono eseguiti o seguiti;
formula e cached value rimangono distinti e nessuna formula viene ricalcolata.
...
In particolare, `mtime` e `ctime` del filesystem non sono evidenze temporali.
I segnali vengono classificati come indipendenti, correlati, duplicati o di
bassa qualità: due segnali correlati non valgono come due conferme indipendenti.
...
La release 1.1 ammette i tipi `candidate_fact`, `candidate_relation`,
`candidate_mapping`, `candidate_conflict`, `candidate_question` e
`temporal_interval`. Solo fatti, relazioni e intervalli temporali hanno una
materializzazione corrente; gli altri tipi rimangono nel candidate registry.
...
Il prossimo step principale è invece la **review** comune. Possiamo però, prima di affrontarla, dedicare un breve step specifico al ramo AI per rendere evidente come nasce e poi si ricongiunge al flusso principale.

----

sì, grazie, esploriamo il ramo AI. per facilitare il processo di ricostruzione del percorso, trattalo come "step <numero che avrebbe dovuto avere nel percorso>" alternativo (o "step <numero> A, B, C..." alternativi, se ce ne vogliono più di uno). torneremo al flusso principale con lo step 4.
...
## Step 3B.1 — DSL Manager prepara un dossier per l’AI
...
## Step 3B.2 — Che cosa riceve realmente l’AI?
...
## Step 3B.3 — L’AI è esterna al nucleo autoritativo
...
## Step 3B.4 — Perché usare l’AI se esistono le regole deterministiche?
...
## Step 3B.5 — L’output rientra tramite una inbox
...
## Step 3B.6 — Che cosa significa “package stale”?
...
## Step 3B.7 — Import e validazione dei candidati AI
...
├── CREC_000031 → valido, pending
├── CREC_000032 → valido, pending
...
## Step 3B.8 — Il punto di ricongiungimento
...
Da qui possiamo tornare al percorso principale con lo **step 4: la review comune**.
...
verrà creato un prompt per produrre design\_document\_v3.md (fortunamente la procedura riguarda un punto secondario e separato della pipeline). proseguiamo sul percorso principale con lo step 4 (integrerò questo tutorial a v3 implementata).
...
```
CREC_000001  database_table CLIENTE
CREC_000002  ORDINE references CLIENTE
CREC_000003  business_rule Cliente/deletion_constraint
```
...
dsl-manager candidates review show <workspace> CREC_000001
```
````
...
Nella release 1.1 l'operazione è disponibile dalla CLI:

```powershell
dsl-manager candidates review correct <workspace> CREC_000004 \
  --actor-id reviewer_anna \
  --reason "relazione corretta" \
  --payload corrections/CREC_000004.json
```
...
```
RDEC_000001: confirmed
      ↓ superseded_by
RDEC_000007: rejected
      ↓ superseded_by
RDEC_000012: confirmed
```
...
RDEC_000001 confirmed ─┐
RDEC_000007 rejected  ─┼──────────→ HEAD = RDEC_000012
RDEC_000012 confirmed ─┘
```
...
```
CREC_000003  → rejected
CREC_000004  → superseded e non più foglia
```
...
```
CBATCH_000001
├── CREC_000001 → confirmed
├── CREC_000002 → pending
├── CREC_000003 → rejected
├── CREC_000004 → superseded
└── CREC_000010 → confirmed, foglia di una correzione
```
...
```
CREC_000020 → rejected
```

Il supporto B rimane confermato:

```
CREC_000031 → confirmed
```
...
La release 1.1 espone la riconciliazione anche come comando esplicito:

```powershell
dsl-manager facts reconcile <workspace>
dsl-manager facts reconcile <workspace> --reconciliation-id RECON_000001 --strict
```
...
└── supporti:
    └── CREC_000001 / FRAG_000001 / confirmed

FACT_000002
├── contenuto: Cliente.deletion_constraint
└── supporti:
    ├── CREC_000020 / CHK_000021 / confirmed
    └── CREC_000031 / CHK_000044 / confirmed

REL_000001
├── contenuto: ORDINE references CLIENTE
└── supporti:
    └── CREC_000002 / FRAG_000007 / confirmed
...
Anno e mese vengono conservati come coverage envelope inclusivi, senza
inventare una precisione giornaliera. I `dateTime` mantengono offset e timezone;
intervalli disgiunti restano spell distinti anziché essere fusi.
...
removed_fact
├── side: before
├── candidate_record_id: CREC_000020
...
added_relation
├── side: after
├── candidate_record_id: CREC_000031
...
> **Limite noto della release 1.1:** il budget complessivo previsto per
> `nodi + archi` GEXF non è ancora applicato dal runtime.
...
Nella release 1.1 il percorso governato completo è disponibile con:

```powershell
dsl-manager batch consolidate <workspace> --path corpus/active
```

Le fasi sono `parse`, `derive`, `review`, `merge` e `reconcile`. Il comando
supporta `--strict-review`, `--reconcile` e la ripresa tramite `--resume`;
senza policy automatiche autorizzate, i candidati restano pending.
...
Il design v02 richiede per i report versione del catalogo, condizione, stato,
outcome, reason, severità, mutazioni, retry, exit code, run, soggetti,
artefatti e contatori. Nella release 1.1 `result_catalog_v1` è completo per
review, derive, merge, reconcile e batch consolidato; preflight OOXML e worker
usano ancora `catalog_version: 1`, mentre i report temporali e GEXF non espongono
l'envelope completo. Le automazioni non devono quindi presumere uniformità. {line\_range\_start=563 line\_range\_end=566 path=.kb/documenti/documenti di design/run 2/design\_document\_v\_02.md git\_url="https://github.com/DanieleBarbiero/dsl\_manager-v1/blob/main/.kb/documenti/documenti di design/run 2/design\_document\_v\_02.md#L563-L566"}
...
renderer
  ↓
snapshot DSL v1/statico o DSL v2/dinamico immutabile
