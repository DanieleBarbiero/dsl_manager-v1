...
def main() -> int:
    ...
    except UnsupportedXmlFormOption as exc:
        _write_error("unsupported_xml_form_option", str(exc), option=exc.option_key)
        return 4
    except (
        XmlFormParserError,
        RuntimeError,
...
def parse(payload: dict[str, Any], *, workspace_dir: Path) -> dict[str, Any]:
    ...
    xml_form_options = _required_dict(payload, "xml_form_options")

    options = parse_xml_form_options(xml_form_options)
    ...
    if actual_source_hash != source_hash:
        raise ValueError(
            ...
            "rerun 'dsl-manager corpus scan' before parsing XML forms."
        )

    xml_text = source_path.read_text(encoding="utf-8")
    parse_result = parse_xml_form_text(xml_text, options)
    ...
    report_path = output_dir / "xml_form_report.json"
    ...
    xml_form_objects = parse_result.to_objects()
    edit_relations = parse_result.edit_relations()
    table_column_references = [
        reference
        for form in parse_result.forms
        for reference in form.table_column_references
    ]
    output_payload = {
        "button_count": parse_result.button_count,
        "edit_relation_count": parse_result.edit_relation_count,
        "edit_relations": edit_relations,
        "exit_code": 0,
        "field_count": parse_result.field_count,
        "form_count": parse_result.form_count,
        ...
        "profile": profile,
        "required_field_count": parse_result.required_field_count,
        "run_id": run_id,
        ...
        "table_reference_count": parse_result.table_reference_count,
        "warnings": list(parse_result.warnings),
        "worker_config": worker_config,
        "worker_name": WORKER_NAME,
        "worker_version": worker_version,
        "xml_form_objects": xml_form_objects,
        "xml_form_options": xml_form_options,
        "xml_form_report_path": relative_workspace_path(workspace_dir, report_path),
    }
...
def _report_payload(output: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "status": "completed",
        "table_column_references": output["table_column_references"],
