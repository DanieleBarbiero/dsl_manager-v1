...
- il presente prompt governa l'esecuzione operativa della Slice 21;
...
- i documenti pertinenti della Slice 21:
...
2. confronta il perimetro e le precondizioni della Slice 21 con design v02 e matrice di tracciabilità;
...
### Tracciabilità minima della Slice 21

Prima del codice costruisci una checklist `requisito → implementazione prevista → test previsto` usando **tutte** le righe della sezione 17 del design assegnate alla Slice 21. Come minimo considera:

- regole DDL/XML/code/log → `test_slice_21_rule_matrix`
- placeholder semantici proibiti → `test_slice_21_unresolved_template_rejected`
...
1. dopo il preflight, implementa soltanto il perimetro della Slice 21;
...
## Integrazioni operative specifiche — Slice 21

- Prima di implementare le nuove regole, dimostra che importer comune, `CandidateReviewService`, migrazione v7, validator, lineage e merge eligibility della Slice 20 esistono realmente e sono coperti da test.
- Inventaria le strutture dati realmente prodotte dai parser delle Slice 12–14; le regole devono consumare quelle strutture, non una forma ipotetica ricostruita dal report.
- Non aggiungere logica di orchestrazione batch: essa appartiene alla Slice 22.
- Nel report produci la matrice `rule → input schema → evidence locator → candidate type → default review state → policy`, includendo tutte le regole della Slice 21 e il comportamento su evidenza insufficiente/placeholder.

## Regola finale di accettazione

La Slice 21 può essere dichiarata `completata` soltanto se soddisfa **sia** il nucleo normativo v02 **sia** il protocollo operativo comune e le integrazioni specifiche sopra. Le integrazioni operative possono rafforzare verifiche e audit, ma non possono ampliare il perimetro funzionale, spostare feature fra Slice, rendere opzionale un requisito del design o introdurre fallback non autorizzati.
