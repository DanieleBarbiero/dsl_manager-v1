...
## 3. Preparare PowerShell e il progetto

Aprire PowerShell nella root di `dsl_manager-v1`.
...
$ZIP = (Resolve-Path ".\corpus_mock_aurora_prestiti.zip").Path

New-Item -ItemType Directory -Path $WS
Expand-Archive -LiteralPath $ZIP -DestinationPath $WS
```
...
```powershell
Get-ChildItem -LiteralPath $WS
Get-ChildItem -LiteralPath (Join-Path $WS "corpus\active") -Recurse -File
```

Prima di processare le fonti è utile leggere:

```powershell
Get-Content -LiteralPath (Join-Path $WS "LEGGIMI_PRIMA.md") -Encoding utf8
Import-Csv -LiteralPath (Join-Path $WS "materiale_di_supporto\inventario_fonti.csv") -Delimiter ";"
```
...
La normalizzazione usa Docling in modalità senza immagini e può richiedere diversi minuti, soprattutto alla prima esecuzione. Non chiudere PowerShell mentre lavora.
...
```powershell
Get-Content -LiteralPath (Join-Path $WS "artifacts\runs\RUN_000001\batch_report.json") -Encoding utf8
```
...
```powershell
Get-ChildItem -LiteralPath (Join-Path $WS "normalized") -Recurse -File
```
...
```powershell
Get-ChildItem -LiteralPath (Join-Path $WS "chunks") -Recurse -Filter "chunks.jsonl"
```
...
```powershell
Get-ChildItem -LiteralPath (Join-Path $WS "fragments") -Recurse -Filter "fragments.jsonl"
```
...
Tornare a un prompt PowerShell libero e creare un package unico con tutte le evidenze attive:
...
```powershell
Get-Content -LiteralPath (Join-Path $WS "ai\outbox\AIPKG_000001\instructions.md") -Encoding utf8
Get-Content -LiteralPath (Join-Path $WS "ai\outbox\AIPKG_000001\content.md") -Encoding utf8
```
...
```powershell
Copy-Item `
  -LiteralPath (Join-Path $WS "ai\outbox\AIPKG_000001\output_template.jsonl") `
  -Destination (Join-Path $WS "ai\inbox\AIPKG_000001_candidates.jsonl")
```
...
```powershell
Get-Content -LiteralPath (Join-Path $WS "exports\dsl\DSL_000001.md") -Encoding utf8
```
...
```powershell
$MANUALE = Join-Path $WS "corpus\active\documenti\vecchi_utili\manuale_pratiche_2012.txt"
Copy-Item -LiteralPath $MANUALE -Destination "$MANUALE.bak"
Add-Content -LiteralPath $MANUALE -Value "`nNota di revisione locale: limite storico da verificare." -Encoding utf8
& $PY -m dsl_mngr corpus scan $WS
```
...
```powershell
Copy-Item -LiteralPath "$MANUALE.bak" -Destination $MANUALE -Force
Remove-Item -LiteralPath "$MANUALE.bak"
& $PY -m dsl_mngr corpus scan $WS
```
...
I file del corpus e della guida sono UTF-8. In PowerShell usare `Get-Content -Encoding utf8` e salvare le risposte AI in UTF-8.
