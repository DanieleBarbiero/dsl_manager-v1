...
def preflight_ooxml(
    ...
    *,
    ...
    with package:
        ...
        if root_relationships is None or workbook_relationships is None or workbook_root is None:
        ...
        for name, root in sorted(retained_roots.items()):
            ...
            if relationship_count > limits.max_relationships:
                raise _budget(
                    "excel.max_relationships", limits.max_relationships
                )
        ...
        return OoxmlPreflightResult(
            ...
            part_content_types=tuple(
                sorted(
                    (name, overrides.get(name) or defaults.get(_part_extension(name)) or "")
                    for name in names
...
def _read_member(
    ...
    *,
    ...
    try:
        ...
        raise _security(f"OOXML member could not be read safely: {info.filename!r}.") from exc
...
def _parse_xml(data: bytes, name: str) -> ElementTree.Element:
    ...
    if b"<!DOCTYPE" in upper or b"<!ENTITY" in upper:
        raise _security(f"DTD or entity declaration is forbidden in OOXML part {name!r}.")
    try:
...
def _select_workbook(
    ...
    required_type = (
        XLSM_WORKBOOK_CONTENT_TYPE if extension == ".xlsm" else XLSX_WORKBOOK_CONTENT_TYPE
    )
    forbidden_type = (
        XLSX_WORKBOOK_CONTENT_TYPE if extension == ".xlsm" else XLSM_WORKBOOK_CONTENT_TYPE
    )
    matching = sorted(name for name, value in overrides.items() if value == required_type)
    if len(matching) != 1:
        raise _security(
...
def _is_xml_part(
    ...
    return name.endswith((".xml", ".rels", ".vml")) or content_type.endswith("+xml") or content_type in {
        "application/xml",
        "text/xml",
    }
...
def _owner_part_for_relationships(name: str) -> str:
    ...
    return owner_name if str(owner_parent) == "." else f"{owner_parent.as_posix()}/{owner_name}"
...
def _normalize_internal_target(owner_part: str, target: str) -> str:
    ...
    try:
        ...
        raise _security("An internal relationship target is not a valid URI reference.") from exc
    if split.scheme or split.netloc or split.query:
        raise _security("An internal relationship target must remain inside the package.")
...
def _validate_workbook_sheets(
    ...
    sheets = [element for element in workbook_root.iter() if element.tag.rsplit("}", 1)[-1] == "sheet"]
    for sheet in sheets:
        ...
        for sheet_meta in sheets_meta:
            ...
            try:
                ...
                if "max_regions" in str(exc):
                    raise _budget("excel.max_regions", preflight.limits.max_regions) from exc
            ...
            for _event, element in ElementTree.iterparse(stream, events=("end",)):
                ...
                if local_name == "workbookPr":
                    ...
                    if not relationship_id or name is None:
                        raise _security("A workbook sheet is missing name or relationship id.")
                    ...
                    if visibility is None:
                        raise _security(f"Unsupported worksheet visibility: {raw_visibility!r}.")
                    sheets.append(
                        {
                            "index": len(sheets),
                            "name": name,
                ...
                for event, element in ElementTree.iterparse(
                    ...
                    if event == "start" and local_name == "table":
                        if table_attributes is not None:
                            raise _security("An OOXML table part contains multiple tables.")
                        table_attributes = {
                            _local_name(key): value
        ...
        try:
            for event, element in ElementTree.iterparse(stream, events=("start", "end")):
                ...
                if event == "start" and local_name == "cellXfs":
                    ...
                    try:
                        custom_formats[int(element.attrib["numFmtId"])] = element.attrib[
                            "formatCode"
                        ]
                    except (KeyError, ValueError) as exc:
                        raise _security("A number format declaration is invalid.") from exc
                    element.clear()
    ...
    *,
    ...
    if raw_type == "s":
        try:
            ...
            raise _security("A shared-string cell references an invalid index.") from exc
    if raw_type == "b":
    ...
    *,
    ...
    if date_system == "1904":
        ...
        if day >= 60:
            day -= 1
    microseconds = int(
        (fraction * Decimal(86_400_000_000)).to_integral_value()
    )
...
def _hash_member(
    ...
    try:
        ...
        raise _security(f"OOXML part could not be hashed safely: {part_name!r}.") from exc
